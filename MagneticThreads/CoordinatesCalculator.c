...
position_vector * concatanate_b_field(position_vector * b_field[],int length){
    int thread;
    position_vector * total_b_field = NULL;
    position_vector * total_b_start;
    
    for(thread = 0; thread < length; thread++){
        total_b_field = b_field[thread];
        if(thread == 0) total_b_start = total_b_field;
        //iterate to end of total
        while(total_b_field != NULL){
            total_b_field = total_b_field->next;
        }
    }
    return total_b_start;
}

int validate_matrix(double lowx, double lowy,double lowz, double highx, double highy, double highz, double xi, double yi, double zi){
    if((xi <= 0) || (yi <= 0) || (zi <= 0)){
        return 0;
    }
